...
try:
    ...
    # Fallback for testing without emulator file
    class PicoEmulator:
        def __init__(self):
            self.pc = 0
            self.sp = 65535
...
# Dracula-inspired Dark Theme Palette
COLORS = {
    "bg": "#282a36",
    "fg": "#f8f8f2",
    "sidebar": "#21222c",
    "current_line": "#44475a",
    "executing_line": "#005f00",  # Dark Green for active instruction
    "error_line": "#6b1818",  # Dark Red for runtime errors
    "comment": "#6272a4",
    "cyan": "#8be9fd",
    "green": "#50fa7b",
    "orange": "#ffb86c",
...
# Instruction Reference Data for Auto-Complete/Help
OPCODE_REF = [
...
class IconFactory:
    """
    Generates vector icons programmatically to avoid external asset dependencies.
    """

    @staticmethod
    def draw_icon(shape, color_hex):
        """
        Draws a specific shape with the given color on a QPixmap.

        Args:
            shape (str): 'play', 'pause', 'step', 'reset', 'settings', 'save', 'open'.
...
class SettingsDialog(QDialog):
    """
    Modal dialog for configuring IDE preferences.
    """
    ...
    def __init__(self, parent=None, current_settings=None):
        ...
        # Stylesheet specifically for the dialog
        self.setStyleSheet(
            f"""
            QDialog {{
        ...
        self.btn_reset_layout.setToolTip(
            "Restores the Dock panels to default positions."
        )
        ...
        # Footer label
        footer = QLabel("by Stefan Jovanovic")
    ...
    def do_reset_layout(self):
        if self.parent_ref:
            ...
            QMessageBox.information(
                self, "Layout Reset", "UI Layout restored to defaults."
            )
...
class LineNumberArea(QWidget):
    """
    Side widget to display line numbers and breakpoints.
    """
    ...
    def mousePressEvent(self, event):
        # Allow toggling breakpoints by clicking the margin
...
class CodeEditor(QPlainTextEdit):
    """
    Enhanced text editor with syntax highlighting, auto-completion,
    and line execution highlighting.
    """
    ...
    def setup_completer(self):
        """Initializes the auto-completion list based on Opcodes."""
    ...
    def set_execution_line(self, line_idx):
        """Highlights the current line being executed by the emulator."""
    ...
    def set_error_line(self, line_idx):
        """Highlights a line that caused a runtime error."""
    ...
    def highlight_lines(self):
        """Applies color formatting to current line, execution line, and errors."""
        extra_selections = []
    ...
    def lineNumberAreaPaintEvent(self, event):
        """Custom painting for line numbers and breakpoint circles."""
        ...
        while block.isValid() and top <= event.rect().bottom():
            if block.isVisible() and bottom >= event.rect().top():
                ...
                # Draw Execution Arrow or Error Mark
                if (
...
class AssemblyHighlighter(QSyntaxHighlighter):
    """
    Syntax highlighter for picoComputer assembly.
    """
...
class MainWindow(QMainWindow):
    """
    The main application window coordinating the Editor, Emulator, and Docks.
    """

    def __init__(self):
        super().__init__()
        self.setWindowTitle("pycoComputer IDE V4")
        self.resize(1300, 850)

        # Settings management
        self.settings_store = QSettings("PicoSystems", "PicoIDE")
        self.app_settings = {"lock_editor": True}
        ...
        self.pc_to_line_map = {}  # Maps Emulator PC to Editor Line Number
    ...
    def load_settings(self):
        """Restores window geometry, dock layout, and last code."""
    ...
    def save_settings(self):
        """Persists window state and code."""
    ...
    def apply_theme(self):
        """Sets the Qt Stylesheet for the application."""
        self.setStyleSheet(
            f"""
    ...
    def setup_toolbar(self):
        ...
        act_reset = QAction(
            IconFactory.draw_icon("reset", COLORS["red"]), "Reset", self
        )
    ...
    def setup_docks(self):
        # --- RIGHT DOCK: INSPECTOR (Memory View) ---
        ...
        # Table showing Address, Symbol Name, Unsigned Val, Signed Val
        self.table_mem = QTableWidget(0, 4)
        self.table_mem.setHorizontalHeaderLabels(["Addr", "Name", "Unsigned", "Signed"])
        ...
        # --- RIGHT DOCK: STACK VIEWER ---
        self.dock_stack = QDockWidget("Stack (SP)", self)
        self.dock_stack.setAllowedAreas(Qt.RightDockWidgetArea | Qt.LeftDockWidgetArea)
        self.dock_stack.setObjectName("dock_stack")

        self.tree_stack = QTreeWidget()
        self.tree_stack.setHeaderLabels(["Address", "Value", "Offset"])
        self.tree_stack.setColumnWidth(0, 80)
        self.tree_stack.setColumnWidth(1, 80)

        self.dock_stack.setWidget(self.tree_stack)
        self.addDockWidget(Qt.RightDockWidgetArea, self.dock_stack)

        # Split inspector and stack vertically
        self.splitDockWidget(self.dock_inspector, self.dock_stack, Qt.Vertical)

        # --- BOTTOM DOCK: TERMINAL & TOOLS ---
    ...
    def reset_layout_defaults(self):
        """Restores docked widgets to their default configuration."""
        self.dock_inspector.setVisible(True)
        self.dock_stack.setVisible(True)
        ...
        self.resizeDocks(
            [self.dock_inspector, self.dock_stack], [400, 300], Qt.Vertical
        )
    ...
    def insert_instruction(self, item, col):
        """Double-clicking reference inserts opcode into editor."""
    ...
    def load_program(self):
        """
        Parses source code and prepares the emulator.

        Returns:
            bool: True if parsing succeeded, False if errors found.
        """
    ...
    def build_sourcemap(self, code_text):
        """
        Maps emulated memory addresses back to source code line numbers
        for highlighting execution flow.
        """
        self.pc_to_line_map = {}
    ...
    def toggle_run(self):
        """Starts, Pauses, or Resumes execution."""
        # 1. Start Session if needed (Parse Code)
        ...
        if self.timer.isActive():
            ...
            if self.emu.is_finished:
                # If finished, auto-reset to start
            ...
            # Check if starting on a breakpoint
    ...
    def step_execution(self):
        """
        Called by Timer or Manual Step.
        Checks breakpoints, performs emulation step, and updates UI.
        """
        # Check Breakpoints
        curr_line = self.pc_to_line_map.get(self.emu.pc, -1)
    ...
    def reset_program(self):
        """Stops execution and reloads the program from scratch."""
        self.timer.stop()
        self.was_running_before_input = False
        self.is_running_session = False
        self.editor.set_execution_line(-1)
    ...
    def update_ui(self):
        """Refreshes all UI components based on Emulator state."""
    ...
    def update_inspector(self):
        """Updates the Memory Inspector table with changed values."""
        # Get all relevant memory locations (Symbols + Modified Addresses)
        ...
        for row, addr in enumerate(sorted_addrs):
            ...
            val_signed = 0
            try:
                # Handle List or Dict memory implementation safely
                if isinstance(self.emu.memory, list):
                    if addr < len(self.emu.memory):
                        val_unsigned = self.emu.memory[addr]
                else:
                    val_unsigned = self.emu.memory.get(addr, 0)

                # Calculate Signed 16-bit
                val_signed = val_unsigned
                if val_signed > 32767:
                    val_signed = val_signed - 65536
            ...
            item_u = QTableWidgetItem(str(val_unsigned))
            item_u.setForeground(QColor(COLORS["cyan"]))

            # Signed column (Read Only)
            item_s = QTableWidgetItem(str(val_signed))
            item_s.setForeground(QColor(COLORS["pink"]))
            item_s.setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)

            self.table_mem.setItem(row, 0, item_addr)
            self.table_mem.setItem(row, 1, item_name)
            self.table_mem.setItem(row, 2, item_u)
            self.table_mem.setItem(row, 3, item_s)

        self.table_mem.blockSignals(False)

    def update_stack_ui(self):
        """Refreshes the Stack Viewer Dock based on the SP register."""
        self.tree_stack.clear()
        sp = self.emu.sp

        # Show top 20 items on the stack relative to SP
        # Stack grows DOWN (SP decrements on push), so active data is at SP+1, SP+2...
        # We visualize range [SP ... SP+20]
        start_addr = sp
        end_addr = min(65536, sp + 20)

        for addr in range(start_addr, end_addr):
            val = self.emu.memory[addr]
            offset_val = addr - sp
            offset_str = f"SP + {offset_val}"

            item = QTreeWidgetItem([str(addr), str(val), offset_str])
            item.setForeground(0, QBrush(QColor(COLORS["comment"])))
            item.setForeground(1, QBrush(QColor(COLORS["cyan"])))
            item.setForeground(2, QBrush(QColor(COLORS["pink"])))

            # Highlight Top of Stack
            if addr == sp:
                item.setBackground(0, QBrush(QColor(COLORS["current_line"])))
                item.setBackground(1, QBrush(QColor(COLORS["current_line"])))
                item.setBackground(2, QBrush(QColor(COLORS["current_line"])))
                item.setText(2, "TOP (SP)")
                item.setForeground(2, QBrush(QColor(COLORS["green"])))
                item.setFont(2, QFont("Consolas", 10, QFont.Bold))

            self.tree_stack.addTopLevelItem(item)

    def handle_memory_edit(self, item):
        """Allows user to edit memory values in the inspector directly."""
        # Only allow editing the "Unsigned" column (Column 2)
        if item.column() != 2:
            return
        ...
        try:
            ...
            # Trigger update to refresh the Signed column
            self.update_inspector()
        except:
            pass

    def handle_input(self):
        """Processes user input from the bottom terminal."""
        ...
        if self.emu.provide_input(text):
            ...
            # Resume execution if no more input needed
            if self.emu.input_needed == 0:
                self.input_field.setEnabled(False)
                self.input_field.setStyleSheet(
                    f"background-color: {COLORS['input_bg']}; color: {COLORS['fg']};"
    ...
    def load_default_code(self):
        """Loads a sample GCD program if no file is found."""
        code = """; picoComputer Example: Euclidean Algorithm
M = 12
N = 5
R = 3
ORG 10

IN M, 2      ; Input M & N from terminal
LOOP: 
DIV R, M, N  ; R = M / N
MUL R, R, N  ; R = int(M/N) * N
SUB R, M, R  ; R = M - R (Remainder)
MOV M, N     ; M = N
MOV N, R     ; N = R
BGT R, 0, LOOP
OUT M, 1     ; Output GCD
